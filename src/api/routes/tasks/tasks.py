...
def update_task(task_id):
    ...
    try:
        updated_task = task_service.update_task(task, data, user_id=user_id)
...
def delete_task(task_id):
    ...
    task_service.delete_task(task, user_id=user_id)
    return ('', 204)
...
def complete_task(task_id):
    ...
    try:
        ...
        if toggle:
            completed_task = task_service.toggle_task_completion(task, user_id=user_id)
        else:
            completed_task = task_service.complete_task(task, user_id=user_id)
...
def uncomplete_task(task_id):
    ...
    try:
        uncompleted_task = task_service.uncomplete_task(task, user_id=user_id)
...
def activate_task(task_id):
    ...
    try:
        activated_task = task_service.activate_task(task, user_id=user_id)
...
def deactivate_task(task_id):
    ...
    try:
        deactivated_task = task_service.deactivate_task(task, user_id=user_id)
...
def change_task_status(task_id):
    ...
    try:
        updated_task = task_service.change_status(task, data['status'], user_id=user_id)
        return jsonify(updated_task.as_dict())
    except TaskValidationError as e:
        return jsonify({'error': e.message}), 400
...
def detach_from_project(task_id):
    ...
    try:
        detached_task = task_service.detach_from_project(task, user_id=user_id)
