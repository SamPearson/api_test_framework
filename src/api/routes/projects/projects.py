...
def update_project(project_id: int):
    ...
    try:
        ...
        updated = service.update_project(project, data, user_id=user_id)
...
def complete_project(project_id: int):
    ...
    try:
        completed = service.complete_project(project, user_id=user_id)
...
def uncomplete_project(project_id: int):
    ...
    uncompleted = service.uncomplete_project(project, user_id=user_id)
...
def activate_project(project_id: int):
    ...
    try:
        activated = service.activate_project(project, user_id=user_id)
...
def deactivate_project(project_id: int):
    ...
    try:
        deactivated = service.deactivate_project(project, user_id=user_id)
...
def change_project_status(project_id: int):
    ...
    try:
        updated = service.change_status(project, data['status'], user_id=user_id)
        return jsonify(updated.as_dict())
    except ProjectValidationError as e:
        return jsonify({'error': e.message}), 400
...
def delete_project(project_id: int):
    ...
    service.delete_project(project, user_id=user_id)
    return ('', 204)
