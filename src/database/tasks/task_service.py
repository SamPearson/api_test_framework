...
class TaskService:
    ...
    def update_task(self, task: Task, data: Dict, user_id: int) -> Task:
    ...
    def delete_task(self, task: Task, user_id: int) -> None:
        """Delete a task and cascade delete all associated records"""

        # Delete the task itself
        self.repository.delete(task, user_id=user_id)

    def complete_task(self, task: Task, user_id: int) -> Task:
        """Mark a task as completed with timestamp"""
        if task.completed:
            return task
        return self.repository.mark_completed(task, user_id=user_id)

    def uncomplete_task(self, task: Task, user_id: int) -> Task:
        """Mark a task as not completed, clearing timestamp"""
        if not task.completed:
            return task
        return self.repository.mark_incomplete(task, user_id=user_id)

    def toggle_task_completion(self, task: Task, user_id: int) -> Task:
        """Toggle the completion status of a task"""
        if task.completed:
            return self.uncomplete_task(task, user_id=user_id)
        return self.complete_task(task, user_id=user_id)

    # Keep these for backwards compatibility, delegating to new methods
    def mark_completed(self, task: Task, user_id: int) -> Task:
        """Mark a task as completed"""
        return self.complete_task(task, user_id=user_id)

    def mark_incomplete(self, task: Task, user_id: int) -> Task:
        """Mark a task as incomplete"""
        return self.uncomplete_task(task, user_id=user_id)

    def activate_task(self, task: Task, user_id: int) -> Task:
        """Activate a task (set active=true)"""
        if task.active:
            return task
        return self.repository.update(task, user_id=user_id, active=True)

    def deactivate_task(self, task: Task, user_id: int) -> Task:
        """Deactivate a task (set active=false)"""
        if not task.active:
            return task
        return self.repository.update(task, user_id=user_id, active=False)

    def change_status(self, task: Task, new_status: str, user_id: int) -> Task:
        ...
        return self.repository.update(task, user_id=user_id, status=new_status)
    ...
    def attach_to_project(self, task: Task, project_id: int, user_id: int) -> Task:
        ...
        return self.repository.set_project(task, project_id, user_id=user_id)

    def detach_from_project(self, task: Task, user_id: int) -> Task:
        """Detach a task from its project (make it standalone)"""
        return self.repository.set_project(task, None, user_id=user_id)
