...
class TaskRepository(UserOwnedRepository[Task]):
    ...
    def update(self, task: Task, user_id: int, title: Optional[str] = None,
        ...
        return super().update(task, user_id=user_id, **update_data)

    def set_project(self, task: Task, project_id: Optional[int], user_id: int) -> Task:
    ...
    def mark_completed(self, task: Task, user_id: int) -> Task:
        ...
        return super().update(task, completed=True, completed_at=datetime.utcnow(), user_id=user_id)

    def mark_incomplete(self, task: Task, user_id: int) -> Task:
        """Mark a task as incomplete, clearing timestamp"""
        return super().update(task, completed=False, completed_at=None, user_id=user_id)