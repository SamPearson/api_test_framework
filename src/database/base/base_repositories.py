    ...
    def update(self, instance: T, user_id: int, **data) -> T:
        ...
        try:
            if not hasattr(instance, 'user_id'):
                raise RepositoryError(f"Permission denied: {self.model_class.__name__} does not support user ownership")

            if int(instance.user_id) != int(user_id):
                raise RepositoryError(
                    f"Permission denied: {self.model_class.__name__} does not belong to user {user_id}")
